(ns razorpay.main
  "Kotodama WASM entrypoint for the Razorpay clean-room actor (L4) — Clojure port.
  ...
  facts (`razorpay.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "razorpay")
(def tier "L4")
...
(def entity-specs
  [{:entity "Payment"   :plural "payments"   :id-prefix "razorpay_pay"
    :fields [:amount :currency :status :method :orderId :email :contact :captured :createdAt] :required [:amount]
    :coerce {:amount :int :captured :bool}          :refs {:orderId "Order"}}
   {:entity "Order"      :plural "orders"      :id-prefix "razorpay_ord"
    :fields [:amount :currency :status :receipt :attempts :createdAt] :required [:amount]
    :coerce {:amount :int :attempts :int}          :refs {}}
   {:entity "Refund"     :plural "refunds"     :id-prefix "razorpay_ref"
    :fields [:amount :currency :status :speed :paymentId :createdAt] :required [:amount]
    :coerce {:amount :int}          :refs {:paymentId "Payment"}}
   {:entity "Customer"   :plural "customers"   :id-prefix "razorpay_cus"
    :fields [:name :email :contact :createdAt] :required [:name]
    :coerce {}                       :refs {}}
   {:entity "Settlement" :plural "settlements" :id-prefix "razorpay_set"
    :fields [:amount :status :fees :tax :createdAt] :required [:amount]
    :coerce {:amount :int :fees :int :tax :int}          :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"razorpay.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "razorpay-compat" :tier tier :entities entities} 200])
...
(defn start! [] :razorpay-compat/ready)
